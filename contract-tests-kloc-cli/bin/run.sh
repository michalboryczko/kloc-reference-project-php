...
show_help() {
    echo -e "${CYAN}kloc-cli Contract Tests Runner${NC}"
    echo ""
    ...
    echo ""
    echo "Test suites: smoke, resolve, usages, deps, context, owners, inherit, overrides"
    echo ""
    echo "Examples:"
    echo "  bin/run.sh test"
    echo "  bin/run.sh test --filter test_resolve_exact_match"
    echo "  bin/run.sh test --suite smoke"
    echo "  bin/run.sh docs"
    echo "  bin/run.sh docs --format=json"
}
...
check_docker() {
    ...
    if ! docker info &> /dev/null 2>&1; then
        echo -e "${RED}Error: Docker is required but not running${NC}"
...
run_tests() {
    ...
    echo -e "${GREEN}=== kloc-cli Contract Tests ===${NC}"
    echo ""
    ...
    local cmd="pytest -v --tb=short --junitxml=output/junit.xml"
    if [[ -n "$filter" ]]; then
        cmd="$cmd -k $filter"
    fi
    if [[ -n "$suite" ]]; then
    ...
    docker compose run --rm contract-tests-kloc-cli $cmd
