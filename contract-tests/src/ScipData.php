...
/**
 * Immutable wrapper around parsed SCIP data with indexed lookups.
 *
 * This class loads the SCIP portion from the unified index.json
...
final class ScipData
{
    ...
    /**
     * Load SCIP data from the unified index.json file.
     *
     * Reads the file and extracts the 'scip' portion containing
     * metadata and documents.
     *
     * @throws RuntimeException if file doesn't exist or can't be read
     * @throws JsonException if JSON is invalid
     */
    public static function loadFromUnifiedJson(string $path): self
    {
        if (!file_exists($path)) {
        ...
        if (!isset($data['scip'])) {
            throw new RuntimeException(
                sprintf('index.json does not contain a "scip" key at: %s', $path)
            );
        }

        return self::fromArray($data['scip']);
    }
